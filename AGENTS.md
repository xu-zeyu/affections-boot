...
- `affections-boot-common`：公共能力、基础组件、常量、异常、工具类
- `affections-boot-auth`：认证鉴权能力
- `affections-boot-start/*`：starter 类基础设施模块
...
- 基础模块：`affections-boot-common`、`affections-boot-auth`
- 基础设施 starter：`affections-boot-starter-{capability}`
- 业务父工程：`jinHan-gold`
...
- 正确：`affections-boot-starter-oss`
...
- `com.affectionsboot.common`
- `com.affectionsboot.auth`
- `com.affectionsboot.starter.{capability}`
...
- 当前仓库同时存在 `com.affectionsboot` 与 `com.jinHan.gold` 两套包前缀，属于既有事实。
- 新增代码不得擅自引入第三套顶级包前缀。
...
- 模块：`affections-boot-starter-oss`
- 包：`com.affectionsboot.starter.oss`
- 配置类：`OssConfig`
- 服务类：`OssService`
